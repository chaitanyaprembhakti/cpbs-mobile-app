...
// Removed strict versioning dependency, replaced with build-time hash parsing mechanism
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Events: Start empty or from local storage.
  const [events, setEvents] = useState<EventData[]>(() => {
    try { return JSON.parse(localStorage.getItem('cpbs_events') || '') || STATIC_EVENTS; } catch { return STATIC_EVENTS; }
  });
  ...
  // --- Persistence Effects ---
  useEffect(() => {
      if (!isLoading && books.length > 0) localStorage.setItem('cpbs_all_books', JSON.stringify(books));
  }, [books, isLoading]);

  useEffect(() => {
      if (!isLoading && lectures.length > 0) localStorage.setItem('cpbs_all_lectures', JSON.stringify(lectures));
  }, [lectures, isLoading]);

  useEffect(() => {
      if (!isLoading && events.length > 0) localStorage.setItem('cpbs_events', JSON.stringify(events));
  }, [events, isLoading]);
  ...
  // Initial Load & Automatic Update Detection
  useEffect(() => {
    const startTime = Date.now();

    // 1. Calculate hashes of current hardcoded data
    const currentBhajanHash = hashString(RAW_BHAJAN_DATA.substring(0, 5000) + RAW_BHAJAN_DATA.length);
    const currentBookHash = hashString(JSON.stringify(BOOKS_DATA).substring(0, 5000));
    const currentLectureHash = hashString(JSON.stringify(LECTURES_DATA).substring(0, 5000));
    const currentStaticEventsHash = hashString(JSON.stringify(STATIC_EVENTS).substring(0, 5000));

    // 2. Retrieve previously saved hashes
    const savedBhajanHash = localStorage.getItem('cpbs_hash_bhajans');
    const savedBookHash = localStorage.getItem('cpbs_hash_books');
    const savedLectureHash = localStorage.getItem('cpbs_hash_lectures');
    const savedStaticEventsHash = localStorage.getItem('cpbs_hash_static_events');

    // 3. Process Bhajans
    if (savedBhajanHash !== currentBhajanHash) {
        ...
        localStorage.setItem('cpbs_hash_bhajans', currentBhajanHash);
    } else {
    ...
    // 4. Process Books
    if (savedBookHash !== currentBookHash) {
        setBooks(BOOKS_DATA);
        localStorage.setItem('cpbs_hash_books', currentBookHash);
    } else {
        try {
            const savedBooks = JSON.parse(localStorage.getItem('cpbs_all_books') || '');
            if(savedBooks && savedBooks.length > 0) setBooks(savedBooks);
            else setBooks(BOOKS_DATA);
        } catch { setBooks(BOOKS_DATA); }
    }

    // 5. Process Lectures
    if (savedLectureHash !== currentLectureHash) {
        setLectures(LECTURES_DATA);
        localStorage.setItem('cpbs_hash_lectures', currentLectureHash);
    } else {
        try {
            const savedLectures = JSON.parse(localStorage.getItem('cpbs_all_lectures') || '');
            if(savedLectures && savedLectures.length > 0) setLectures(savedLectures);
            else setLectures(LECTURES_DATA);
        } catch { setLectures(LECTURES_DATA); }
    }

    // 6. Process Static Events (Calendar fallback)
    if (savedStaticEventsHash !== currentStaticEventsHash) {
        // Only override if the current events are the old static events or empty.
        // If they've fetched live events recently, we keep those, but update hash
        setEvents(prev => {
            if (prev.length === 0 || JSON.stringify(prev) === localStorage.getItem('cpbs_old_static_events')) {
                return STATIC_EVENTS;
            }
            return prev;
        });
        localStorage.setItem('cpbs_old_static_events', JSON.stringify(STATIC_EVENTS));
        localStorage.setItem('cpbs_hash_static_events', currentStaticEventsHash);
    }
  ...
  const importData = (json: string): boolean => {
    try {
        ...
        if (updated) {
            ...
            // Invalidate hash so next load knows we've modified data manually
            localStorage.setItem('cpbs_hash_bhajans', 'admin_modified');
        }
        if (data.books) {
            setBooks(data.books);
            localStorage.setItem('cpbs_hash_books', 'admin_modified');
        }
        if (data.lectures) {
            setLectures(data.lectures);
            localStorage.setItem('cpbs_hash_lectures', 'admin_modified');
        }
        if (data.events) {
  ...
  const resetData = () => {
       ...
       localStorage.removeItem('cpbs_hash_bhajans');
       localStorage.removeItem('cpbs_hash_books');
       localStorage.removeItem('cpbs_hash_lectures');
       localStorage.removeItem('cpbs_hash_static_events');
  ...
  const saveBhajanLocally = (id: string, title: string, content: string) => {
      setBhajans(prev => {
          ...
          // Invalidate hash so local edits persist
          localStorage.setItem('cpbs_hash_bhajans', 'user_modified');
          return newBhajans;
      });
