...
import { Play, Pause, Music, Loader2, X, Check, Mic2, ChevronsUpDown, RotateCcw, RotateCw, Download, Repeat, Repeat1, MessageCircle } from 'lucide-react';
...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({ 
  ...
  const handleRequestAudio = () => {
      const message = `Hare Krishna! Please add audio for this bhajan:\n\n*${bhajanTitle}*`;
      window.open(`https://wa.me/917974422740?text=${encodeURIComponent(message)}`, '_blank');
  };
    ...
    <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-saffron-200 dark:border-slate-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] pb-[env(safe-area-inset-bottom)] z-50 transition-all duration-300 relative landscape:border-t-0 landscape:h-full landscape:flex landscape:flex-col landscape:justify-between landscape:pb-4">
      ...
      <div className="p-2 flex items-center justify-between gap-2 landscape:flex-col landscape:h-full landscape:justify-center landscape:gap-6">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center px-2 landscape:w-full landscape:px-0 landscape:items-center">
            ...
            {audioTracks.length > 0 ? (
                ...
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-1.5 px-3 bg-saffron-50 dark:bg-slate-800 text-saffron-600 dark:text-saffron-400 rounded-lg border border-saffron-200 dark:border-slate-700 hover:bg-saffron-100 dark:hover:bg-slate-700 transition-colors"
                >
        ...
        <button 
            ...
            disabled={!activeTrack || isDownloaded || isDownloading}
            className={`p-2.5 rounded-full transition-colors ${
                !activeTrack ? 'opacity-0 pointer-events-none' :
                isDownloaded
