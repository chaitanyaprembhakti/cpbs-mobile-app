...
import { X, Info, MessageCircle, Home, Download, ShieldCheck, CircleHelp, Smartphone, Play, Heart, Bell } from 'lucide-react';
...
export const SideMenu: React.FC<SideMenuProps> = ({ 
  isOpen, onClose, onOpenAbout, onOpenDonate, onHome, onOpenDownloaded, onOpenPrivacy, onOpenInformation, onOpenOnboarding, scrollBarSide = 'left', settingsLanguage 
}) => {
  const { informationData, hasNewInformation } = useSettings();
  const { showToast } = useToast();
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [showHelpModal, setShowHelpModal] = useState(false);
  const [showFeedbackModal, setShowFeedbackModal] = useState(false);
  const [appVersion, setAppVersion] = useState('v8.0.0');
  ...
  const t = {
    en: {
      ...
      noNewInformation: "No new information",
      donate: "Donate",
    ...
    hi: {
      ...
      home: "होम",
      information: "सूचना (Information)",
      noNewInformation: "कोई नई सूचना नहीं",
      donate: "सहयोग करें",
  ...
  const handleInformationClick = () => {
    if (onOpenInformation) onOpenInformation();
  };
            ...
            <MenuItem 
              icon={
                <div className="relative">
                  ...
                  {hasNewInformation && (
                    <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#fdfbf7] dark:border-slate-900 animate-pulse" />
                  )}
                </div>
              } 
              label={t.information} 
