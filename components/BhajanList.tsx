...
import { ChevronRight, Music, Filter, Flame, CloudRain, Bell, Book, Gift, X, Check, Sun, Moon, Plus, ListMusic, Languages, Search } from 'lucide-react';
...
export const BhajanList: React.FC<BhajanListProps> = ({ 
  bhajans, onSelect, searchQuery, onSearchChange, script, indexMode, onIndexModeChange, azSliderSide, playlists, onCreatePlaylist, scrollBarSide = 'left', settingsLanguage = 'en', onSettingsLanguageChange
}) => {
  ...
  const t = {
    ...
    searchPlaceholder: settingsLanguage === 'hi' ? "खोजें (Hinglish).." : "Search (Hinglish)...",
  };
  ...
  return (
    <div className="relative pb-2">
      ...
      {(isSearchOpen || searchQuery.trim().length > 0) && (
        <div className={`sticky top-0 z-30 bg-[#fdfbf7]/95 dark:bg-slate-900/95 backdrop-blur-md py-2 border-b border-[#bc8d31]/20 flex items-center gap-2 ${headerPaddingClass} animate-in slide-in-from-top-2 duration-200`}>
             <Search className="text-[#bc8d31]" size={20} />
             <input 
               autoFocus
               className="flex-1 bg-transparent outline-none py-2 text-slate-900 dark:text-white placeholder:text-slate-400 font-medium"
      ...
      {!searchQuery.trim() && !isSearchOpen && activeFilter !== 'all' && (
        <div className={`${stickyClass} bg-[#fdfbf7]/95 dark:bg-slate-900/95 backdrop-blur-md py-3 border-b border-[#bc8d31]/20 flex justify-between items-center ${headerPaddingClass}`}>
           ...
           <div className="flex items-center gap-2">
               <button 
                  ...
                  className="p-2 rounded-full transition-all active:scale-95 bg-[#bc8d31]/10 text-[#bc8d31] dark:bg-slate-800 hover:bg-[#bc8d31]/20"
               >
                   <Search size={16} />
               </button>
               <button 
      ...
      {!searchQuery.trim() && !isSearchOpen && activeFilter === 'all' && sortedKeys.length > 0 && (
        <div className={`sticky top-0 z-20 flex justify-end h-0 overflow-visible pointer-events-none ${headerPaddingClass}`}>
           <div className="py-2 pointer-events-auto flex items-center gap-2">
               <button 
                  onClick={() => {
                      triggerHaptic('light');
                      setIsSearchOpen(true);
