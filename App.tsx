...
const BhajanReader = React.lazy(() =>
  import("./components/BhajanReader").then((module) => ({
    default: module.BhajanReader,
  })),
);
const BookReader = React.lazy(() =>
  import("./components/BookReader").then((module) => ({
...
const AboutScreen = React.lazy(() =>
  import("./components/AboutScreen").then((module) => ({
    default: module.AboutScreen,
  })),
);
const DonateScreen = React.lazy(() =>
  import("./components/DonateScreen").then((module) => ({
    default: module.DonateScreen,
  })),
);
const DownloadedList = React.lazy(() =>
  import("./components/DownloadedList").then((module) => ({
    default: module.DownloadedList,
  })),
);
const DailyQuotes = React.lazy(() =>
  import("./components/DailyQuotes").then((module) => ({
    default: module.DailyQuotes,
  })),
);
const EventsScreen = React.lazy(() =>
  import("./components/EventsScreen").then((module) => ({
    default: module.EventsScreen,
  })),
);
const DailyDarshanScreen = React.lazy(() =>
  import("./components/DailyDarshanScreen").then((module) => ({
    default: module.DailyDarshanScreen,
  })),
);
const SevaCenterScreen = React.lazy(() =>
  import("./components/SevaCenterScreen").then((module) => ({
    default: module.SevaCenterScreen,
  })),
);
const CategoryList = React.lazy(() =>
  import("./components/CategoryList").then((module) => ({
    default: module.CategoryList,
  })),
);
const QuotePopup = React.lazy(() =>
  import("./components/QuotePopup").then((module) => ({
    default: module.QuotePopup,
  })),
);
const LibraryScreen = React.lazy(() =>
  import("./components/LibraryScreen").then((module) => ({
    default: module.LibraryScreen,
  })),
);
const PrivacyPolicyScreen = React.lazy(() =>
  import("./components/PrivacyPolicyScreen").then((module) => ({
    default: module.PrivacyPolicyScreen,
  })),
);
const DailyInformationScreen = React.lazy(() =>
  import("./components/DailyInformationScreen").then((module) => ({
    default: module.DailyInformationScreen,
  })),
);
const ActivitiesScreen = React.lazy(() =>
  import("./components/ActivitiesScreen").then((module) => ({
    default: module.ActivitiesScreen,
  })),
);
const InformationScreen = React.lazy(() =>
  import("./components/InformationScreen").then((module) => ({
    default: module.InformationScreen,
  })),
);
const GuruMaharajScreen = React.lazy(() =>
  import("./components/GuruMaharajScreen").then((module) => ({
    default: module.GuruMaharajScreen,
  })),
);
const VirahiMaharajScreen = React.lazy(() =>
  import("./components/VirahiMaharajScreen").then((module) => ({
    default: module.VirahiMaharajScreen,
  })),
);
...
import { setupPushNotifications } from "./services/firebase";
...
const MainApp: React.FC = () => {
  ...
  const [animDirection, setAnimDirection] = useState<"left" | "right" | "none">(
    "none",
  );
  const [searchQuery, setSearchQuery] = useState("");
  const [debouncedQuery, setDebouncedQuery] = useState("");
  const [selectedBhajan, setSelectedBhajan] = useState<Bhajan | null>(null);
  const [selectedContextQueue, setSelectedContextQueue] = useState<
    BhajanAudio[] | undefined
  >(undefined);
  ...
  const [isGuruMaharajOpen, setIsGuruMaharajOpen] = useState(false);
  const [isVirahiMaharajOpen, setIsVirahiMaharajOpen] = useState(false);
  const [showQuotePopup, setShowQuotePopup] = useState(false);
  const [isForcedUpdateInProgress, setIsForcedUpdateInProgress] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const configureNative = async () => {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          await setupPushNotifications((data) => {
            if (data.type === "information") {
              window.history.pushState(
                { isPopup: true, view: "information" },
                "",
              );
        ...
        try {
          ...
          if (result.updateAvailability === 2) {
            ...
            setIsForcedUpdateInProgress(true);
        ...
        try {
          ...
          await NotificationService.createChannel(); // Ensure channel is created
          await NotificationService.scheduleDailyQuote();

          // 1. Listener for standard local notification click (if any)
          const localNotificationHandle = await LocalNotifications.addListener(
            "localNotificationActionPerformed",
            (notification) => {
              const extra = notification.notification.extra;
              if (extra?.type === "daily_quote") {
                window.history.pushState({ isPopup: true, view: "quotes" }, "");
                setIsDailyQuotesOpen(true);
              }
            },
          );

          // 2. Listener for our Custom BigPictureNotification click
          const bigPictureHandle = await BigPictureNotification.addListener(
            "notificationActionPerformed",
            (data) => {
              if (data.action === "open_quotes") {
                window.history.pushState({ isPopup: true, view: "quotes" }, "");
                setIsDailyQuotesOpen(true);
              } else if (data.action === "open_info") {
                window.history.pushState(
                  { isPopup: true, view: "information" },
                  "",
                );
          ...
          return () => {
            localNotificationHandle?.remove?.();
            bigPictureHandle?.remove?.();
          };
        } catch (e) {
    ...
    configureNative().then((cleanupFunc) => {
      cleanup = cleanupFunc;
    });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const lastSeenAfternoon = localStorage.getItem(
        "cpbs_quote_last_seen_afternoon",
      );

      const shouldShow =
        lastSeenDate !== todayStr ||
        (currentHour >= 16 && lastSeenAfternoon !== todayStr);
      ...
      if (shouldShow) {
        ...
        img.onload = () => {
          ...
          if (currentHour >= 16)
            localStorage.setItem("cpbs_quote_last_seen_afternoon", todayStr);
        };
  ...
  // Global PopState Listener for Browser Back / History Sync
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (isSettingsOpen && view !== "settings") setIsSettingsOpen(false);
      if (isAboutOpen && view !== "about") setIsAboutOpen(false);
      if (isDonateOpen && view !== "donate") setIsDonateOpen(false);
      if (isSideMenuOpen && view !== "menu") setIsSideMenuOpen(false);
      if (isDailyQuotesOpen && view !== "quotes") setIsDailyQuotesOpen(false);
      if (isDailyInfoOpen && view !== "info") setIsDailyInfoOpen(false);
      if (isEventsOpen && view !== "events") setIsEventsOpen(false);
      if (
        isDailyDarshanOpen &&
        view !== "darshan" &&
        view !== "darshan-category" &&
        view !== "darshan-image"
      )
        setIsDailyDarshanOpen(false);
      if (isSevaCenterOpen && view !== "seva") setIsSevaCenterOpen(false);
      if (isActivitiesOpen && view !== "activities") setIsActivitiesOpen(false);
      if (isPrivacyOpen && view !== "privacy") setIsPrivacyOpen(false);
      if (isInformationOpen && view !== "information")
        setIsInformationOpen(false);
      if (isGuruMaharajOpen && view !== "guru-maharaj")
        setIsGuruMaharajOpen(false);
      if (isVirahiMaharajOpen && view !== "virahi-maharaj")
        setIsVirahiMaharajOpen(false);

      if (hasSelectedBhajan && view !== "reader") {
        setSelectedBhajan(null);
        setSelectedContextQueue(undefined);
        setIsNewBhajan(false);
      }
      if (hasSelectedBook && view !== "book") setSelectedBook(null);
    };
  ...
  const closeTopmostView = useCallback(() => {
    ...
    // Forced update blocks back button completely
    if (isForcedUpdateInProgress) return true;

    // Onboarding blocks back button unless completed
    if (showOnboarding) return true;

    if (showLandscapeSearch) {
      setShowLandscapeSearch(false);
      return true;
    }
    if (showQuotePopup) {
    ...
    const checkAndClose = (
      ...
      if (isOpen) {
        if (window.history.state?.view === viewName) window.history.back();
        else closeFn();
        return true;
      }
      return false;
    ...
    if (hasSelectedBhajan) {
      if (window.history.state?.view === "reader") window.history.back();
      else {
        setSelectedBhajan(null);
        setSelectedContextQueue(undefined);
        setIsNewBhajan(false);
    ...
    if (hasSelectedBook) {
      if (window.history.state?.view === "book") window.history.back();
      else setSelectedBook(null);
      return true;
    }

    if (
      checkAndClose(isDailyQuotesOpen, "quotes", () =>
        setIsDailyQuotesOpen(false),
      )
    )
      return true;
    if (checkAndClose(isDailyInfoOpen, "info", () => setIsDailyInfoOpen(false)))
      return true;
    ...
    if (isDailyDarshanOpen) {
      ...
      if (
        ...
        window.history.back();
      else setIsDailyDarshanOpen(false);
      return true;
    }

    if (
      checkAndClose(isSevaCenterOpen, "seva", () => setIsSevaCenterOpen(false))
    )
      return true;
    if (
      checkAndClose(isActivitiesOpen, "activities", () =>
        setIsActivitiesOpen(false),
      )
    )
      return true;
    if (checkAndClose(isPrivacyOpen, "privacy", () => setIsPrivacyOpen(false)))
      return true;
    if (
      checkAndClose(isInformationOpen, "information", () =>
        setIsInformationOpen(false),
      )
    )
      return true;
    if (
      checkAndClose(isGuruMaharajOpen, "guru-maharaj", () =>
        setIsGuruMaharajOpen(false),
      )
    )
      return true;
    if (
      checkAndClose(isVirahiMaharajOpen, "virahi-maharaj", () =>
        setIsVirahiMaharajOpen(false),
      )
    )
      return true;
    if (checkAndClose(isEventsOpen, "events", () => setIsEventsOpen(false)))
      return true;
    if (checkAndClose(isDonateOpen, "donate", () => setIsDonateOpen(false)))
      return true;
    if (
      checkAndClose(isSettingsOpen, "settings", () => setIsSettingsOpen(false))
    )
      return true;
    if (checkAndClose(isAboutOpen, "about", () => setIsAboutOpen(false)))
      return true;
    if (checkAndClose(isSideMenuOpen, "menu", () => setIsSideMenuOpen(false)))
      return true;
  ...
  useEffect(() => {
    ...
    const backButtonListener = CapApp.addListener(
      ...
      ({ canGoBack }) => {
        ...
        if (handledInternally === true) {
          exitAttemptRef.current = 0;
          return;
        }

        if (handledInternally === "tab_switch") {
  ...
  const handleOpenLecture = (
    ...
    if (existingBhajan) {
      ...
      if (targetTrack && existingBhajan.audio) {
        const idx = existingBhajan.audio.findIndex(
          (t) => t.id === targetTrack.id,
        );
        if (idx !== -1) {
          queue = [
            ...existingBhajan.audio.slice(idx),
            ...existingBhajan.audio.slice(0, idx),
          ];
        }
      }
    ...
    if (targetTrack && lecture.audio) {
      ...
      if (idx !== -1) {
        const rawQueue = [
          ...lecture.audio.slice(idx),
          ...lecture.audio.slice(0, idx),
        ];
        queue = rawQueue.map((t) => ({
          ...t,
          bhajanTitle: lecture.title,
          parentBhajan: undefined,
        }));
      }
    } else if (lecture.audio && lecture.audio.length > 0) {
      queue = lecture.audio.map((t) => ({
        ...t,
        bhajanTitle: lecture.title,
        parentBhajan: undefined,
      }));
  ...
  const handleDownloadedItemSelect = (item: any) => {
    if (item.fileName) {
      handleOpenBook(item as Book);
    } else if (item.description && item.date) {
      handleOpenLecture(item as LectureData);
    } else {
      handleOpenReader(item as Bhajan);
    }
  };
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books
      .map((b) => ({ book: b, score: calculateBookScore(b, debouncedQuery) }))
      .filter((item) => item.score > 0)
      .sort((a, b) => b.score - a.score)
      .map((item) => item.book);
  ...
  const { filteredKatha, filteredKirtan } = useMemo(() => {
    const audioBhajans = bhajans
      .filter((b) => b.audio && b.audio.length > 0)
      .map((b) => ({
        id: b.id,
        title: script === "iast" ? b.titleIAST : b.title,
        description: script === "iast" ? b.contentIAST : b.content,
        date: "Kirtan",
        audio: b.audio,
      }));
    if (!debouncedQuery.trim())
      return { filteredKatha: lectures, filteredKirtan: audioBhajans };
    const scoreAndSort = (items: LectureData[]) =>
      items
        .map((l) => ({
          lecture: l,
    ...
    return {
      filteredKatha: scoreAndSort(lectures),
      filteredKirtan: scoreAndSort(audioBhajans),
    };
  ...
  const searchPlaceholder =
    settingsLanguage === "hi" ? "खोजें (Hinglish).." : "Search (Hinglish)...";
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-paper dark:bg-slate-950 overflow-hidden transition-colors duration-500 relative">
      ...
      {!selectedBhajan && currentBhajan && (
        <MiniPlayer onExpand={() => handleOpenReader(currentBhajan)} />
      )}
      ...
      <SideMenu
        ...
        onClose={goBack}
        onOpenAbout={() => {
          window.history.pushState({ isPopup: true, view: "about" }, "");
          setIsAboutOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenDonate={() => {
          window.history.pushState({ isPopup: true, view: "donate" }, "");
        ...
        onOpenDownloaded={() => {
          window.history.replaceState({ view: "downloaded" }, "");
        ...
        onOpenDailyInfo={() => {
          window.history.pushState({ isPopup: true, view: "info" }, "");
          setIsDailyInfoOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenEvents={() => {
          window.history.pushState({ isPopup: true, view: "events" }, "");
          setIsEventsOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenDailyDarshan={() => {
          window.history.pushState({ isPopup: true, view: "darshan" }, "");
          setIsDailyDarshanOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenSevaCenter={() => {
          window.history.pushState({ isPopup: true, view: "seva" }, "");
          setIsSevaCenterOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenPrivacy={() => {
          window.history.pushState({ isPopup: true, view: "privacy" }, "");
          setIsPrivacyOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenInformation={() => {
          window.history.pushState({ isPopup: true, view: "information" }, "");
          setIsInformationOpen(true);
          setIsSideMenuOpen(false);
        }}
        onOpenOnboarding={() => {
          window.history.pushState({ isPopup: true, view: "onboarding" }, "");
      ...
      <Suspense fallback={<LoadingFallback />}>
        ...
        {isSettingsOpen && (
          <SettingsScreen
            ...
            lectures={lectures}
            events={events}
            onUpdateBhajans={setBhajans}
            onUpdateBooks={setBooks}
            onUpdateLectures={setLectures}
        ...
        {isDailyDarshanOpen && (
          <DailyDarshanScreen onBack={goBack} scrollBarSide={scrollBarSide} />
        )}
        {isSevaCenterOpen && (
          <SevaCenterScreen
            onBack={goBack}
        ...
        {isInformationOpen && informationData && (
          <InformationScreen
        ...
        {isGuruMaharajOpen && <GuruMaharajScreen onBack={goBack} />}
        {isVirahiMaharajOpen && <VirahiMaharajScreen onBack={goBack} />}
        ...
        {selectedBhajan && (
          <BhajanReader
            ...
            script={script}
            darkMode={darkMode}
            onToggleTheme={() => handleThemeChange(!darkMode)}
            keepAwake={keepAwake}
            devMode={devMode}
            scrollBarSide={scrollBarSide}
            onSave={saveBhajanLocally}
        ...
        {activeTab === "downloaded" && (
          <div className="fixed inset-0 z-40 bg-white dark:bg-slate-900 animate-slide-in-bottom">
            <DownloadedList
              allBhajans={bhajans}
              onSelect={handleDownloadedItemSelect}
              onBack={goBack}
              script={script}
              scrollBarSide={scrollBarSide}
            />
      ...
      <Header
        onOpenMenu={() => {
          window.history.pushState({ isPopup: true, view: "menu" }, "");
          setIsSideMenuOpen(true);
        }}
        onOpenSettings={() => {
          window.history.pushState({ isPopup: true, view: "settings" }, "");
      ...
      {showLandscapeSearch && (
        <div className="fixed top-2 left-20 right-20 z-40 bg-white/95 dark:bg-slate-800/95 p-2 rounded-xl shadow-xl flex items-center gap-2 animate-in fade-in slide-in-from-top-2 border border-slate-200 dark:border-slate-700">
          ...
          <input
            ...
            onBlur={() => {
              if (!searchQuery) setShowLandscapeSearch(false);
            }}
          />
          <button
            onClick={() => setShowLandscapeSearch(false)}
      ...
      <main
        ref={mainScrollRef}
        className={`relative z-10 flex-1 overflow-y-auto w-full max-w-3xl mx-auto scroll-container pt-[calc(4rem+env(safe-area-inset-top))] pb-[calc(6rem+env(safe-area-inset-bottom))] landscape:pt-[env(safe-area-inset-top)] landscape:pb-[env(safe-area-inset-bottom)] landscape:pl-[calc(4rem+env(safe-area-inset-left))] landscape:pr-[calc(5rem+env(safe-area-inset-right))] landscape:max-w-none ${
          scrollBarSide === "left" ? "left-scrollbar" : ""
        }`}
        ...
        >
          {activeTab === "home" && (
            <HomeScreen
              ...
              onOpenGuruMaharaj={() => {
                window.history.pushState(
                  { isPopup: true, view: "guru-maharaj" },
                  "",
                );
                setIsGuruMaharajOpen(true);
              }}
              onOpenVirahiMaharaj={() => {
                window.history.pushState(
                  { isPopup: true, view: "virahi-maharaj" },
                  "",
                );
                setIsVirahiMaharajOpen(true);
              }}
              settingsLanguage={settingsLanguage}
            />
          )}
          ...
          >
            {activeTab === "authors" && (
              <CategoryList
                bhajans={filteredBhajans}
                onSelect={(b) => handleOpenReader(b, undefined, "Authors List")}
                script={script}
              />
            )}
            {activeTab === "books" && (
